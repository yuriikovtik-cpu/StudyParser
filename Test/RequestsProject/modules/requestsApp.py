...
def getTools():
    ...
    try:
        product['memory'] = soup.find('span', string="Вбудована пам'ять").find_next_sibling('span').text.strip()
    except AttributeError:
        product['memory'] = None
    ...
    try:
        for comment_block in soup.find_all('div', class_='br-comment-text loaded'):
            ...
            if p:
                product['comment'].append(p.text.strip())
    except AttributeError:
    ...
    try:
        ...
        for item in items:
            category_name = item.find('h3').get_text().strip()
            specifications[category_name] = {}
            rows = item.find('h3').find_next_sibling('div').find_all('div', recursive=False)
            for row in rows:
                spans = row.find_all('span')
                if len(spans) >= 2:
                    key = spans[0].get_text().strip()
                    value = spans[1].get_text(" ").strip().replace('\xa0', ' ')
    ...
    df = pd.DataFrame([flat_product])
    df.to_csv(r'D:\Python\PycharmProjects\Test\RequestsProject\results\productsRequests.csv', index=False)
